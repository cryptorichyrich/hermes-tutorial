...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        [data-theme="dark"] .sr-item-snippet mark {{
            background: #ca8a04; color: #fef9c3;
        }}
        @media (prefers-color-scheme: dark) {{
            :root:not([data-theme]) .sr-item-snippet mark {{
                background: #ca8a04; color: #fef9c3;
            }}
        }}
        .sr-empty {{
        ...
        .theme-toggle:hover svg {{ stroke: var(--ink); }}
        .theme-toggle .sun-icon {{ display: none; }}
        .theme-toggle .moon-icon {{ display: block; }}
        [data-theme="dark"] .theme-toggle .sun-icon {{ display: block; }}
        [data-theme="dark"] .theme-toggle .moon-icon {{ display: none; }}
        @media (prefers-color-scheme: dark) {{
            :root:not([data-theme]) .theme-toggle .sun-icon {{ display: block; }}
        ...
        [data-theme="dark"] .mermaid-zoom-overlay {{
            background: rgba(0,0,0,0.85);
        }}
        @media (prefers-color-scheme: dark) {{
            :root:not([data-theme]) .mermaid-zoom-overlay {{
                background: rgba(0,0,0,0.85);
            }}
        }}
        .mermaid svg {{
        ...
        .mermaid-zoom-controls .zoom-level {{
            ...
            letter-spacing: 0.3px; min-width: 48px; justify-content: center;
        }}
...
<body>
    ...
    <script>
        ...
        // Mermaid zoom with pan/drag + zoom buttons
        document.querySelectorAll('.mermaid').forEach(function(m) {{
            m.addEventListener('click', function() {{
                ...
                btnOut.innerHTML = '&#8722;'; btnOut.title = 'Zoom out';
                var btnIn = document.createElement('button');
                btnIn.innerHTML = '&#43;'; btnIn.title = 'Zoom in';
                var btnReset = document.createElement('button');
                btnReset.innerHTML = '&#8634;'; btnReset.title = 'Reset';
                var level = document.createElement('span');
                level.className = 'zoom-level'; level.textContent = '100%';
                ...
                diagramWrap.innerHTML = m.innerHTML;
                content.appendChild(diagramWrap);
                overlay.appendChild(content);
                document.body.appendChild(overlay);

                function applyScale() {{
                    var svg = diagramWrap.querySelector('svg');
                ...
                overlay.addEventListener('mousemove', function(e) {{
                    if (!isPanning) return;
                    content.scrollLeft = scrollL - (e.pageX - content.offsetLeft - startX);
                    content.scrollTop = scrollT - (e.pageY - content.offsetTop - startY);
                }});
